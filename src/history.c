 ...
 *	  the get_command_key_sequence() routine so that illegal keystrokes (too many
 ...
 *	  scrolling to work)
 *	- made lookup_file() look first in the $HOME directory, then in the
 ...
 *	- added TTflush in bind_to_key() to force space out to terminal after
 *	  binding name.
 ...
 *	- changed KEY_BINDING structure to allow us to bind user macroes
 *	  (buffers) to keys as well as C functions. Lots of changes
 *	  for this.
 *	18-jan-89
 *	- finished changes for the new KEY_BINDING structure.
 ...
 *	- orginized the , VIOD and const keywords better.
 ...
 *	- command_key_sequence_to_string() returns char*.
 ...
 *	  I changed the lookup_file() search order to look in the current
 ...
 *	3 Jan 93
 *	- eliminated a redundant declaration in estruct.h for
 ...
 *	  and command_key_sequence_to_string() returns it as ^@ so we can see it.
 ...
 *  - rename OBJECT type to UEOBJECT (UE for �Emacs) to avoid collision
